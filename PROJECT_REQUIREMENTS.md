...
FarmPrism is an implemented agricultural marketplace prototype connecting farmers, buyers and logistics partners. Current phase: [2.0.13 — functional My Farm and clean activity](PHASE_2_0_13.md). The current local Development working tree and latest explicit product decisions are authoritative. Phase files preserve historical implementation and validation records; they do not override current requirements.
...
Physical inventory represents the supported crop count. Current batches have positive remaining KG and status other than sold/completed/cancelled. Current crops and active batch counts derive from these rows; available totals and crop available KG include only status available. My Farm name remains null. Activities derive batch creation and actual update timestamps; no historical Farm Updated event is fabricated. Sold This Month, New Offer, Top Opportunity and market calculations retain the existing rules. No inventory produces honest zero/empty data.

## Phase 2.0.13 My Farm writes and clean activity

All My Farm secondary screens are functional through authenticated Farmer-only Node endpoints: POST /api/farmer/crops, POST /api/farmer/batches and PATCH /api/farmer/farm. Actor identity comes only from the validated session. Add Crop creates the first positive batch; duplicate current crops return CROP_ALREADY_EXISTS. Add Produce requires an existing current crop, returns CROP_NOT_ACTIVE otherwise, and creates a separate batch. No new crop table. Quantity is 0.01–1,000,000 KG with at most two decimal places. New grade is null, source farmer_declared, status available; quality remains Sell-only.

Edit Farm persists only location label, farm area and explicit coordinates. No role, phone, verification, Farmer ID or farm name writes. Omitted coordinates are preserved. Device location uses existing expo-location; external Maps uses saved coordinates via Linking. Missing coordinates produce an honest disabled state. Existing Sell Quality/Price Insight/listing screens remain canonical; SelectBatch supports an optional crop filter.

Prototype OTP intentionally remains any six numeric digits for the nine fixed accounts, without SMS or Supabase Auth users. Clean activity is produced only by the guarded clear:demo-activity CLI with exact CLEAR_FARMPRISM_ACTIVITY confirmation. Preserve 9 accounts, 3 profiles per role, configuration/categories and official mandi_prices rows; clear listed activity tables, sessions and only demo market rows. Never invoke the old reseeding reset for this workflow. See PHASE_2_0_13.md for measured cleanup results and limitations.
