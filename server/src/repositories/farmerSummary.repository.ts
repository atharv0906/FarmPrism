...
export const farmerSummaryRepository: FarmerSummaryRepository = {
  async read(accountId) {
    const [accounts, profiles, batches, orders, notifications] = await Promise.all([
      ...
      rows(supabaseAdmin.from('demo_farmer_profiles').select('account_id,location_label,farm_area_acres,latitude,longitude').eq('account_id', accountId).limit(1000)),
      rows(supabaseAdmin.from('demo_inventory_batches').select('id,batch_code,crop_name,original_quantity_kg,remaining_quantity_kg,quality_grade,status,created_at,updated_at').eq('farmer_account_id', accountId).limit(1000)),
    ...
    return {
      ...
      profile: { location: profiles[0].location_label === null ? '' : text(profiles[0].location_label), area: profiles[0].farm_area_acres === null ? null : number(profiles[0].farm_area_acres), latitude: coordinate(profiles[0].latitude, 90), longitude: coordinate(profiles[0].longitude, 180) },
      batches: batches.map(b => ({ ...parseFarmBatch(b), remainingKg: number(b.remaining_quantity_kg) })),
