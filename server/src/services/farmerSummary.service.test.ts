...
test('Home preserves one batched market read with empty inventory', async () => {
  const data = fixture(); data.batches = []; data.auctions = []; data.bids = [];
  let calls = 0;
  const home = await createFarmerSummaryService({ read: async () => data }, {
    history: async () => { throw new Error('Individual reads must not run'); },
    historyMany: async (crops, days, district) => {
      calls++; assert.deepEqual(crops, ['Tomato', 'Onion', 'Potato']); assert.equal(district, 'Pune');
      return crops.map(crop => ({ crop: crop as Crop, days, points: [point(crop as Crop)], retrievedAt: now.toISOString(), fallback: true }));
    },
  }, () => now).home('farmer');
  assert.equal(calls, 1); assert.equal(home.farm.cropCount, 0); assert.equal(home.farm.availableQuantityKg, 0);
  assert.equal(home.marketPrices.length, 3); assert.ok(home.marketPrices.every(price => price.pricePerQuintal === 2500));
});
