...
export function RoleProvider({children}:PropsWithChildren){
 ...
 const value=useMemo<RoleContextValue>(()=>({loading,availableRoles,selectedRole,error,selectRole:async(roleId)=>{if(!user)return;const assigned=availableRoles.find(r=>r.id===roleId);if(!assigned){setError(new RoleServiceError('not_assigned','The selected role is not assigned to this user.'));return}try{if(isDevelopmentMockOtpEnabled()){await AsyncStorage.setItem(MOCK_LAST_ROLE_KEY,assigned.id);setSelectedRole(assigned);setError(null);return}const role=await roleService.selectRole(user.id,roleId);setSelectedRole(role);setError(null)}catch(e){setError(e instanceof RoleServiceError?e:new RoleServiceError('unknown_error','The application role could not be selected.',e))}},clearSelectedRole:()=>setSelectedRole(null),reloadRoles}),[availableRoles,error,loading,reloadRoles,selectedRole,user]);
