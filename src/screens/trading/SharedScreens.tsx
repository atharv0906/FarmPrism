  ...
  return <Page title={order?.code ?? 'Order Details'} loading={state.loading} error={action.error ?? state.error} retry={() => void state.refresh()}>
    {order ? <>
      <Card title={order.batch.crop + ' · ' + order.batch.code}><Text>{order.quantityKg} KG · {money(order.pricePerKg)}/KG · Total {money(order.total)}</Text>
        ...
        {buyer && ['in_transit', 'delivery_otp_pending'].includes(order.status) && <Button title="Generate Delivery OTP" disabled={action.pending} onPress={() => void action.run(() => mutations.generateDeliveryOtp(order.id, options), r => setOtp({ value: r.data.otp, expiresAt: r.data.expiresAt }))} />}
