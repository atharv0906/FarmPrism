...
export function SelectBatchScreen({ navigation, route }: Props<'SelectBatch'>) {
  const state = useTrading();
  const batches = state.data?.batches.filter(b => (!route.params?.crop || b.crop === route.params.crop) && b.status === 'available' && b.quantityKg > 0 && !state.data?.items.some(i => i.batch.id === b.id && active(i.status))) ?? [];
  return <FarmerPage title="Select Batch" back hasData={!!state.data} loading={state.loading} error={state.error} retry={() => void state.refresh()}>{state.data && !batches.length && <View style={ui.empty}><Text style={ui.emptyTitle}>No available batches</Text><Text style={ui.muted}>Add produce in My Farm or review your active listings.</Text><Button title="Open My Farm" onPress={() => navigation.getParent()?.navigate('MyFarm')} outline /></View>}{batches.map(batch => <Card key={batch.id}><View style={ui.listingTop}><Image source={cropArtwork(batch.crop)} resizeMode="contain" style={ui.cropImage} /><View style={ui.listingCopy}><Text style={ui.cropTitle}>{batch.crop}</Text><Text style={ui.muted}>{batch.code}</Text><Badge>{batch.status}</Badge></View></View><Text style={ui.muted}>{quintals(batch.quantityKg)} · {batch.quantityKg} KG available</Text><Text style={ui.muted}>Farmer Declared Grade {batch.grade ?? 'Not declared'}</Text><Button title="Select Batch" onPress={() => navigation.navigate('Quality', { batchId: batch.id })} /></Card>)}</FarmerPage>;
}
