...
import { Page as TradingPage, Card as TradingCard, Button as TradingButton, money, date } from '../../components/trading/TradingUI';
...
export function SellHomeScreen({ navigation }: Props<'SellHome'>) {
  ...
  return <FarmerPage title="Sell Produce" hero loading={state.loading} error={state.error} retry={() => void state.refresh()}>
...
export function QualityScreen({ route, navigation }: Props<'Quality'>) {
  ...
  return <FarmerPage title="Farmer Declared Quality" back loading={state.loading} error={localError ?? action.error ?? state.error} retry={() => void state.refresh()}>{batch ? <Card><SectionTitle title={`${batch.crop} · ${batch.code}`} icon={cropArtwork(batch.crop)} /><Text style={ui.muted}>Choose the farmer-declared quality for this batch.</Text><View style={ui.toggleRow}>{(['A', 'B', 'C'] as const).map(value => <View key={value} style={[ui.toggle, grade === value && ui.toggleSelected]}><Button title={`Grade ${value}`} onPress={() => setGrade(value)} outline={grade !== value} /></View>)}</View><Field label="Notes (optional)" value={notes} onChange={setNotes} /><Button title={action.pending ? 'Saving…' : 'Continue to Market / Price Insight'} disabled={action.pending || !demoApiToken} onPress={continueToInsight} /></Card> : state.data && <Text style={ui.muted}>Batch unavailable.</Text>}</FarmerPage>;
}
...
export function PriceInsightScreen({ route, navigation }: Props<'PriceInsight'>) {
  ...
  return <FarmerPage title="Price Insight" back loading={state.loading} error={state.error} retry={() => void state.refresh()}>{data && <><Card><SectionTitle title="Market Intelligence" /><Text style={ui.cropTitle}>{data.crop} · Grade {data.quality.grade ?? 'Not declared'}</Text><Text style={ui.source}>{marketSourceLabel(data.market.isDemo, data.market.source)}</Text><View style={ui.heroAside}><View style={ui.trust}><Text style={ui.small}>Current Market Price</Text><Text style={ui.value}>{`₹${data.market.latestModalPricePerKg.toLocaleString('en-IN')}/KG`}</Text></View><View style={ui.trust}><Text style={ui.small}>7-Day Recommendation</Text><Text style={ui.value}>{`₹${data.recommendation.suggestedReservePricePerKg.toLocaleString('en-IN')}/KG`}</Text></View></View><Text style={ui.muted}>Market Range: ₹{data.market.minPricePerKg}/KG – ₹{data.market.maxPricePerKg}/KG</Text><Text style={ui.muted}>Confidence: {data.recommendation.confidence}</Text><Text style={ui.muted}>{data.recommendation.reasoning}</Text></Card><Button title="Choose Selling Method" onPress={() => navigation.navigate('ChooseMethod', { batchId: route.params.batchId, suggestedPrice: data.recommendation.suggestedReservePricePerKg })} /><MarketDetailsBody crop={data.crop} /></>}{state.error && <Button title="Continue with my own selling price" onPress={() => navigation.navigate('ChooseMethod', { batchId: route.params.batchId })} outline />}</FarmerPage>;
}

export function ChooseMethodScreen({ route, navigation }: Props<'ChooseMethod'>) { return <FarmerPage title="Choose Selling Method" back><Card style={ui.actionTile}><SectionTitle title="AUCTION" icon={sellAssets.tomato} /><Text style={ui.muted}>Let buyers compete with bids</Text><Button title="Choose Auction" onPress={() => navigation.navigate('CreateListing', { ...route.params, kind: 'auction' })} /></Card><Card style={ui.actionTile}><SectionTitle title="FIXED PRICE" icon={sellAssets.onion} /><Text style={ui.muted}>Set the price you want</Text><Button title="Choose Fixed Price" onPress={() => navigation.navigate('CreateListing', { ...route.params, kind: 'fixed' })} /></Card></FarmerPage>; }
...
export function CreateListingScreen({ route, navigation }: Props<'CreateListing'>) {
  ...
  return <FarmerPage title={isAuction ? 'Create Auction' : 'Create Fixed Price'} back loading={state.loading} error={action.error ?? state.error} retry={() => void state.refresh()}>{batch && <Card><SectionTitle title={`${batch.crop} · ${batch.code}`} icon={cropArtwork(batch.crop)} /><Text style={ui.muted}>Available: {quintals(batch.quantityKg)} · {batch.quantityKg} KG</Text><Field label="Quantity (KG)" value={quantity} onChange={setQuantity} numeric /><Field label={isAuction ? 'Reserve price (₹/KG)' : 'Fixed price (₹/KG)'} value={price} onChange={setPrice} numeric />{isAuction && <><Text style={ui.fieldLabel}>Auction duration</Text><View style={ui.toggleRow}>{([6, 12, 24] as const).map(d => <PressDuration key={d} selected={duration === d} label={`${d} Hours`} onPress={() => setDuration(d)} />)}</View></>}<Button title={action.pending ? 'Publishing…' : isAuction ? 'Publish Auction' : 'Publish Fixed Price'} disabled={!valid || action.pending || !demoApiToken} onPress={submit} /></Card>}</FarmerPage>;
}

function PressDuration({ selected, label, onPress }: { selected: boolean; label: string; onPress: () => void }) { return <View style={[ui.toggle, selected && ui.toggleSelected]}><Button title={label} onPress={onPress} outline={!selected} /></View>; }
...
function BuyerItemScreen({ item, state, action, demoApiToken, navigation }: { item?: TradingItem; state: ReturnType<typeof useTrading>; action: ReturnType<typeof useTradingAction>; demoApiToken: string | null; navigation: Props<'Item'>['navigation'] }) {
  ...
  return <TradingPage title={item?.kind === 'fixed' ? 'Fixed Listing Details' : 'Auction Details'} loading={state.loading} error={action.error ?? state.error} retry={() => void state.refresh()}>{item ? <><TradingCard title={item.batch.crop + ' · ' + item.batch.code}><Text>Offered {item.offeredKg} KG · Remaining {item.remainingKg} KG</Text><Text>{money(item.pricePerKg)}/KG · {item.status}</Text><Text>Farmer Declared Grade {item.batch.grade ?? 'Not declared'}</Text><Text>Starts {date(item.startsAt)}</Text><Text>Ends {date(item.endsAt)}</Text>{role === 'buyer' && valid && (item.kind === 'fixed' || item.status === 'open') && <TradingButton title={item.kind === 'auction' ? 'Place / Revise Bid' : 'Request Purchase'} onPress={() => navigation.navigate('BidForm', { itemId: item.id })} />}</TradingCard><ProfileCard profile={state.data?.profiles.find(p => p.id === item.batch.farmerId)} /></> : state.data && <Text>This listing is unavailable. Refresh Market or Sell.</Text>}</TradingPage>;
}
...
export function OfferScreen({ route, navigation }: Props<'Offer'>) {
  ...
  return <FarmerPage title="Offer Details" back loading={state.loading} error={action.error ?? state.error} retry={() => void state.refresh()}>{offer && <><ProfileCard profile={state.data?.profiles.find(p => p.id === offer.buyerId)} /><Card><SectionTitle title={offer.kind === 'fixed' ? 'Fixed-price request' : 'Auction bid'} /><Text style={ui.value}>{quintals(offer.remainingKg)}</Text><Text style={ui.muted}>₹{offer.pricePerKg}/KG · {offer.advancePercent}% advance · {offer.status}</Text><Text style={ui.muted}>Delivery: {offer.delivery ?? 'Unavailable'}</Text><Field label={`Accept quantity (KG), maximum ${limit}`} numeric value={quantity} onChange={setQuantity} /><Button title="Use full remaining quantity" onPress={() => setQuantity(String(limit))} outline /><Button title={action.pending ? 'Accepting…' : 'Accept Offer'} disabled={action.pending || !Number.isFinite(q) || q <= 0 || q > limit} onPress={acceptOffer} /><Button title={action.pending ? 'Working…' : offer.kind === 'auction' ? 'Reject Offer' : 'Reject Request'} disabled={action.pending} onPress={() => Alert.alert('Reject this request?', 'This cannot be undone.', [{ text: 'Cancel', style: 'cancel' }, { text: 'Reject', style: 'destructive', onPress: rejectOffer }])} /></Card></>}</FarmerPage>;
}
